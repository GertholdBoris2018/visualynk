...
visualynk = function(visualynk) {
    ...
    a.taxonomy.createTaxonomyPanel = function() {
        ...
        b.append("img").attr("id", function(b){return b.label+"parentImage"}).attr("src", "vendor/plugins/2d_model/img/downArrow.png")
          .attr("width", "15").attr("height", "15").attr('class', 'expanded')
          .on('load', function(b) {
    ...
    a.taxonomy.updateCount = function(b) {
        ...
        (function(b) {
            ...
            a.rest.post({
                ...
                a.logger.error(c +
                    ': error while accessing Neo4j server on URL:"' + a.rest.CYPHER_URL + '" defined in "visualynk.rest.CYPHER_URL" property: ' + d);
    ...
    a.taxonomy.addTaxonomyChildren = function(b) {
        b.each(function(b) {
          if (b.children !== undefined) {
            d3.select("#"+b.label+"childImage").attr("src","vendor/plugins/2d_model/img/downArrow.png").attr("width", "15").attr("height", "15").attr('class', 'collapsed');
          };
            var d = d3.select(this),
                e = b.children;
            b.children && (b = d.append("ul").selectAll("li").data(e).enter().append("li").attr("id", function(a) {
                    return a.id
                }).attr("value", function(a) {
                    return a.label
                })/*.style('display', 'none')*/,
                b.append("img").attr("id", function(b){return b.label+"childImage"}).attr("src", "vendor/plugins/2d_model/img/category.png").style('padding-left', '15px')
                    .attr("width", "12").attr("height", "12").on("click", function(b){
                      if (d3.select("#"+b.label+"childImage").classed('collapsed')) {
                        d3.select("#"+b.label+"childImage").attr("src", "vendor/plugins/2d_model/img/downArrow.png").attr("width", "12")
                          .attr("height", "12").style('padding-right', "15").attr('class', 'expanded');
                          var childIDs = b.children;
                          childIDs.forEach(function(c){
                            return d3.select("#"+c.id).style('display', 'block')
                          })
                      } else if (d3.select("#"+b.label+"childImage").classed('expanded')) {
                        d3.select("#"+b.label+"childImage").attr('class', 'collapsed').attr("src", "vendor/plugins/2d_model/img/downArrow.png").attr("width", "12")
                          .attr("height", "12").attr('class', 'collapsed')
                        var childIDs = b.children;
                        childIDs.forEach(function(c){
                          return d3.select("#"+c.id).style('display', 'none')
                        });
                      }
                      //alert(JSON.stringify(b.children))
                    }),
                b.append("span").attr("class", "vslynk-label").text(function(b) {
                    return a.provider.getTaxonomyTextValue(b.label)
                }), b.append("span").attr("class", "vslynk-count"),
    ...
    a.taxonomy.onClick = function() {
        if ((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPod/i))  || (navigator.userAgent.match(/Android/i)) ) {
            console.log("Mobile device taxonomy clicked" + $(this).attr("id"));
    ...
    a.graph.node.updateCount = function() {
        ...
        a.rest.post({
            ...
            a.logger.error(e + ': error while accessing Neo4j server on URL:"' + a.rest.CYPHER_URL + '" defined in "visualynk.rest.CYPHER_URL" property: ' + f);
    ...
    a.graph.node.addForegroundElements = function(b) {
        ...
        b.append("rect").attr("x", a.graph.node.CountBox.x).attr("y", a.graph.node.CountBox.y).attr("width", a.graph.node.CountBox.w).attr("height", a.graph.node.CountBox.h).attr("class", "vslynk-count-box");
    ...
    a.graph.node.updateMiddlegroundElements = function() {
        ...
        //      EGER NOKTALARA BAGLANTI ATAMAK ISTERSEK BURAYI AKTIF HALE GETIRECEGIZ
    ...
    a.graph.node.mouseOverNode = function() {
        ...
        if (b.type === a.graph.node.NodeTypes.VALUE) {
            textValue = a.provider.getTextValue(b);
            var cleanGUIDNumber1;
            localStorage.setItem('kucukHarfler', ((b.label).toLowerCase()));
            localStorage.setItem('buyukHarfler', (b.label));
            localStorage.setItem('kucukHarflerIs', ((b.label).toLowerCase()));
            localStorage.setItem('buyukHarflerIs', (b.label));
            $.ajax({
                type: "POST",
    ...
    a.graph.node.nodeClick = function() {
        ...
        if (b.type === a.graph.node.NodeTypes.VALUE) {
            ...
            $.ajax({
                ...
                success: function(guidNumber) {
                    var guid = guidNumber.results[0].data[0].row[0].name;
                    cleanGUIDNumber = guid.replace(/\"/g, "");
                    localStorage.setItem("graphGUID", cleanGUIDNumber);
                    //localStorage.setItem("graphGUIDIsicin", cleanGUIDNumber);
                    localStorage.setItem("noktaIsim", textValue);

                    // GRANLUND METRIX ILE BAGLANTI BU SATIRDAN ITIBAREN YAPIILIYOR.
                    if (localStorage.getItem("kucukHarfler") === "space") {
                        var username = 'visual-lynk';
                        var password = 'Q1w2mdE7!o';
                        var gosterilecek_yer = "sag_sonuc"
                        $.ajax({
                            url: "http://rd.granlund.fi/metrix/space/" + localStorage.getItem("graphGUID") + "",
                            type: 'GET',
                            contentType: 'application/json',
                            dataType: 'jsonp',
                            username: username,
                            ...
                            success: function(result) {
                                var myWindow = window.open("", gosterilecek_yer, "width=1200, height=800");
                                myWindow.document.write("<p>" + result + "</p>");
                            }
                        });
                    };
                }
    ...
    a.graph.node.chooseNodeClick = function(b) {
        ...
        a.graph.node.chooseWaiting || b.immutable || (a.graph.force.nodes().forEach(function(b) {
            ...
            a.graph.node.chooseWaiting = !1;
            a.logger.error(d + ': error while accessing Neo4j server on URL:"' + a.rest.CYPHER_URL + '" defined in "visualynk.rest.CYPHER_URL" property: ' + e)
        }))
    };
    ...
    a.queryviewer.createQueryArea = function() {
        ...
        a.queryviewer.queryConstraintSpanElements = d3.select(b).append("p").attr("class", "vslynk-query-constraint-elements").selectAll(".queryConstraintSpan");
        a.queryviewer.querySpanElements = d3.select(b).append("p").attr("class", "vslynk-query-elements").selectAll(".querySpan")
    };
    ...
    a.query.generateTaxonomyCountQuery = function(b) {
        ...
        d.join(" AND ") : "") + " RETURN count(DISTINCT n." + c + ") as count"
    ...
    a.query.generateResultCypherQuery = function() {
        ...
        for (var l = a.provider.getReturnAttributes(b.label), g = a.provider.getConstraintAttribute(b.label), h = 0; h < l.length; h++) {
            ...
            k === a.query.NEO4J_INTERNAL_ID ? k == g ? e.push("ID(" + b.internalLabel + ") AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) : e.push("COLLECT(DISTINCT ID(" + b.internalLabel +
                ")) AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) : k == g ? e.push(b.internalLabel + "." + k + " AS " + k) : e.push("COLLECT(DISTINCT " + b.internalLabel + "." + k + ") AS " + k)
        }
    ...
    a.query.generateResultCypherQueryCount = function() {
        var b = a.graph.getRootNode(),
            ...
            c = c.whereElements,
            l = [];
        d === a.query.NEO4J_INTERNAL_ID ? f.push("count(DISTINCT ID(" + b.internalLabel + ")) AS count") : f.push("count(DISTINCT " + b.internalLabel + "." + d + ") AS count");
    ...
    a.query.generateNodeCountCypherQuery = function(b) {
        ...
        f === a.query.NEO4J_INTERNAL_ID ? e.push("count(DISTINCT ID(" + b.internalLabel + ")) as count") : e.push("count(DISTINCT " + b.internalLabel + "." + f + ") as count");
    ...
    a.query.generateValueQuery = function(b) {
        ...
        for (var g = a.provider.getReturnAttributes(b.label), h = a.provider.getConstraintAttribute(b.label), k = 0; k < g.length; k++) g[k] === a.query.NEO4J_INTERNAL_ID ? g[k] == h ? l.push("ID(" + b.internalLabel + ") AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) : l.push("COLLECT (DISTINCT ID(" + b.internalLabel + ")) AS " + a.query.NEO4J_INTERNAL_ID.queryInternalName) :
            g[k] == h ? l.push(b.internalLabel + "." + g[k] + " AS " + g[k]) : l.push("COLLECT(DISTINCT " + b.internalLabel + "." + g[k] + ") AS " + g[k]);
    ...
    a.result.updateResults = function() {
        if (a.result.hasChanged) {
            ...
            a.rest.post({
                ...
                }), b.enter().append("p").attr("class", "vslynk-result").attr("id", function(a) {
                ...
                a.logger.error(d + ': error while accessing Neo4j server on URL:"' +
                    a.rest.CYPHER_URL + '" defined in "visualynk.rest.CYPHER_URL" property: ' + e);
                a.result.resultListeners.forEach(function(a) {
                    a([])
                })
            });
    ...
    a.provider.getPredefinedConstraints = function(b) {
        console.log("return predefinedConstraints => "+ a.provider.getProperty(b, "getPredefinedConstraints")());
    ...
    a.provider.DEFAULT_PROVIDER = Object.freeze({
        ...
        getTextValue: function(b) {
            ...
            return (b.type === a.graph.node.NodeTypes.VALUE ? c === a.query.NEO4J_INTERNAL_ID ? "" + b.internalID : "" + b.attributes[c] : void 0 === b.value ? b.label : c === a.query.NEO4J_INTERNAL_ID ? "" + b.value.internalID : "" + b.value.attributes[c]).substring(0, a.graph.node.NODE_MAX_CHARS)
        },
