    ...
    function BimSurfer(cfg) {
                ...
                params.api.call("ServiceInterface", "getAllRelatedProjects", {poid: params.poid}, function(data) {
                    if (data.length > 0) {
                        ...
                        for (var index in data) {
                            ...
                            if (projectData.nrSubProjects > 0) continue;
                            params.poids.push(projectData.oid);
                            params.roids.push(projectData.lastRevisionId);
                            params.schemas.push(projectData.schema);
                            
                            self.poids.push(projectData.oid);
                            self.roids.push(projectData.lastRevisionId);
                            self.schemas.push(projectData.schema);
                            self.models.push(projectData);
                        }
                        var projectData = data[0];
                        params.poid = projectData.oid;
        ...
        this._loadFromAPI = function (params) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (params.roids.constructor === Array) {
                    for (index in params.roids) {
                        var models = [];
                        params.api.getModel(params.poids[index], params.roids[index], params.schemas[index], false,
                            function (model) {
                                ...
                                model.query(PreloadQuery,
                                    function () {
                                        if (!fired) {
                                            ...
                                            models.push(vmodel);
                                            if (models.length == params.poids.length) {
                                                resolve(models);
                                            }
                                        }
                                    });
        ...
        this.getColorOpacityBookmark = function(){
            return viewer.getBookmark({colors:true});
        }
         /**
